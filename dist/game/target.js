...
export class Target extends Ball {
    ...
    static create(type, element) {
        ...
        const position = new Vector2(JSONHelper.getRandomInRange(Constant.Ball.Target.Position.X), JSONHelper.getRandomInRange(Constant.Ball.Target.Position.Y));
        const velocity = new Vector2(JSONHelper.getRandomInRange(Constant.Ball.Target.Velocity.X) * Math.sign(Math.random() - 0.5), JSONHelper.getRandomInRange(Constant.Ball.Target.Velocity.Y));
        const acceleration = new Vector2(JSONHelper.getRandomInRange(Constant.Ball.Target.Acceleration.X), JSONHelper.getRandomInRange(Constant.Ball.Target.Acceleration.Y));
        return new Target(radius, mass, position, velocity, acceleration, type, element);
    }
    static copy(other, type, element) {
        const radius = JSONHelper.getByPath(Constant.Ball.Radius, type) ?? Constant.Ball.Radius.Default;
        const mass = JSONHelper.getByPath(Constant.Ball.Mass, type) ?? Constant.Ball.Mass.Default;
