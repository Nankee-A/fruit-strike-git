import { Animator, Constant, GameModule, SVGHelper, Vector2 } from "./base/index.js";
...
export class Scene extends GameModule {
    ...
    _targets = [];
    _bullets = [];
    _bulletGroups = new Map;
    _bowStringAngle = 0;
    _aimLineAnimations = [];
    _maskBlinkAnimations = [];
    _upgradeEffectAnimation;
    _over = false;
    _boundPoints = [
    ...
    _bowStringPoints = [-1, 0, 1].map(num => new Vector2(Constant.Scene.Background.Width / 2 + Constant.Scene.Bow_String.Length / 2 * num, Constant.Scene.Bow_String.Y));
    constructor() {
        super();
    ...
    async onInit() {
        ...
        this._setAttributes(this._gameScene, ["viewBox", `0 0 ${Constant.Scene.Background.Width} ${Constant.Scene.Background.Height}`]);
        this._setAttributes(this._background, ["fill", Constant.Scene.Background.Color]);
        this._setAttributes(this._mask, ["y", Constant.Scene.Mask.Y], ["fill", Constant.Scene.Mask.Color], ["opacity", Constant.Scene.Mask.Opacity]);
        this._setAttributes(this._bound, ["x", Constant.Scene.Bound.Min.X], ["y", Constant.Scene.Bound.Min.Y], ["width", Constant.Scene.Bound.Max.X - Constant.Scene.Bound.Min.X], ["height", Constant.Scene.Bound.Max.Y - Constant.Scene.Bound.Min.Y], ["fill", Constant.Scene.Bound.Color], ["opacity", Constant.Scene.Bound.Opacity]);
        this._setAttributes(this._totalScoreElement, ["fill", Constant.Scene.UI.Total_Score.Color], ["font-size", Constant.Scene.UI.Total_Score.Font_Size]);
        this._setAttributes(this._totalKillElement, ["fill", Constant.Scene.UI.Total_Score.Color], ["font-size", Constant.Scene.UI.Total_Score.Font_Size]);
        this._setAttributes(this._totalScoreElement.querySelector(".title"), ["x", Constant.Scene.UI.Total_Score.Title.X], ["y", Constant.Scene.UI.Total_Score.Title.Y]);
        this._setAttributes(this._totalKillElement.querySelector(".title"), ["x", Constant.Scene.UI.Total_Kill.Title.X], ["y", Constant.Scene.UI.Total_Kill.Title.Y]);
        this._setAttributes(this._totalScoreContent, ["x", Constant.Scene.UI.Total_Score.Title.X + Constant.Scene.UI.Total_Score.Content.X], ["y", Constant.Scene.UI.Total_Score.Title.Y + Constant.Scene.UI.Total_Score.Content.Y]);
        this._setAttributes(this._totalKillContent, ["x", Constant.Scene.UI.Total_Kill.Title.X + Constant.Scene.UI.Total_Kill.Content.X], ["y", Constant.Scene.UI.Total_Kill.Title.Y + Constant.Scene.UI.Total_Kill.Content.Y]);
        this._setAttributes(this._bowString, ["points", this._toPointsString(...this._bowStringPoints)], ["stroke-width", Constant.Scene.Bow_String.Width], ["stroke", Constant.Scene.Bow_String.Color]);
        this._setAttributes(this._aimLine, ["stroke-width", Constant.Scene.Aim_Line.Width], ["stroke", Constant.Scene.Aim_Line.Color], ["opacity", 0]);
        this._setAttributes(this._overElement.querySelector(".content"), ["fill", Constant.Scene.UI.Over.Color], ["font-size", Constant.Scene.UI.Over.Font_Size]);
        var x = Constant.Scene.UI.Health.X - Constant.Scene.UI.Health.Radius * 2;
        for (let i = 0; i < Constant.Game.Health; i++) {
            ...
            this._setAttributes(healthElement, ["x", x], ["y", Constant.Scene.UI.Health.Y], ["width", Constant.Scene.UI.Health.Radius * 2], ["height", Constant.Scene.UI.Health.Radius * 2]);
    ...
    setAimLine(active = false, angle = -0.5 * Math.PI, stretch = 0) {
        if (!active) {
            this._setAttributes(this._aimLine, ["x1", this._bowStringPoints[1].x], ["y1", this._bowStringPoints[1].y], ["x2", this._bowStringPoints[1].x], ["y2", this._bowStringPoints[1].y], ["opacity", 0]);
    ...
    setScore(score, position) {
        ...
        this._setAttributes(scoreElement, ["class", "score"], ["fill", Constant.Scene.UI.Score.Color], ["font-size", Constant.Scene.UI.Score.Font_Size], ["style", "user-select: none; -webkit-user-select: none;"], ["opacity", 1], ["x", position.x], ["y", position.y]);
    ...
    makeHitEffect(position) {
        ...
        this._setAttributes(effectElement, ["class", "hit-effect"], ["cx", position.x], ["cy", position.y], ["fill", Constant.Scene.Effect.Hit.Color], ["opacity", 1], ["r", 0]);
    ...
    makeUpgradeEffect(color) {
        if (this._upgradeEffectAnimation) {
            Animator.kill(this._upgradeEffectAnimation);
            this._resetUpgradeEffect();
        }
        this._setAttributes(this._upgradeEffect, ["fill", color]);
    ...
    _resetUpgradeEffect() {
        this._setAttributes(this._upgradeEffect, ["cx", Constant.Scene.Background.Width / 2], ["cy", Constant.Scene.Bow_String.Y], ["r", 0], ["opacity", 1]);
    }
